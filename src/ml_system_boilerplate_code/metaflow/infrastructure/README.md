# Metaflow Infrastructure Deployment Module
This module uses Terraform and AWS for infrastructure.  

This module contains several submodules, which can be used independently or as one high-level module:  
- AWS Batch Cluster to run the Metaflow steps (modules/computation)  
- Blob storage and metadat database (modules/datastore)  
- A service providing an API to record and query past executions (modules/metadata_service)  
- Some resources to deploy Metaflow flows on Step Function sprocessing (modules/step_functions)  
- Metaflow UI (modules/ui)

### Computation
The computation module sets up a remote computation resource(s) so flows can be run on an AWS EC2 instance(s). These resources do not perform the orchestration and rely on the data scientist's computer to perform this coordination.

### Datastore
The datastore module sets up a blob and tabular data storage, which records all flows, the steps they took, their conda environments, artifacts, and results. This should eist for the lifetime of the stack.

### Metadata Service
The metadata_service module sets up an API entrypoint to interact with all other services, both for running flows and interacting with the Datastore module to explore historic runs.

### Step Functions
The step_functions module sets up remote computation resources that come with orchestration. This allows the data science team to schedule flows using crons as well as being able to kick off flows and shut down their machine when needed, and the remote resources will handle all coordination.  
